...
# Ensure the "ORACLE_JOBS" folder exists relative to the script's location
...
if not os.path.exists(csv_path):
    raise FileNotFoundError("The file 'Websites.csv' was not found in the 'EMERSON_JOBS' folder.")

# Delete all .txt and .csv files in the "ORACLE_JOBS" folder, but keep the Websites.csv file
...
def extract_data_from_text(content):
    ...
    markers = {
        ...
        # Date Posted NOT working
        "Date_Posted": ("Posting Date\n", "\nJob Schedule"),
        
        #Req NOT working
        "Job_Requisition_Number": ("Identification\n", "\nJob Category"),

        "Work_Site": ("Locations\n", "\n\n"),
        # "Travel": ("Travel\n", "\nRole type"),

        #Job Type NOT working
        "Job_Type": ("Job Schedule\n", "\nLocations"),
        # "Profession": ("Profession\n", "\nDiscipline"),

        #Discipline NOT working
        "Discipline": ("Job Category\n", "\nPosting Date"),

        # "Employment_Type": ("Job Schedule\n", "\nLocations"),
        "Minimum_Qualifications": ("\nFor This Role, You Will Need:", ["\nPreferred Qualifications that Set You Apart:", "\nOur Offer To You:"]),
        "Preferred_Qualifications": ("\nPreferred Qualifications that Set You Apart:", "\nOur Offer To You:"),
        # "Salary_Range": ("The typical base pay range for this role across the U.S. is USD ", "per year."),
        "Duties_Functions_and_Responsibilities": ("In this Role, Your Responsibilities Will Be:\n", "\nWho You Are:"),
        "Knowledge_Skills_and_Abilities": ("Who You Are:\n", "\nFor this Role You Will Need:"),
        # Marker for City and Title and extraction
    ...
    for key, marker in markers.items():
        if key == "Posting_Title":
            # Extracting the title by finding the correct line
            lines = content.split('\n')
            title_line_index = 2  # Adjusted index based on our finding
            extracted_data[key] = lines[title_line_index].strip() if len(lines) > title_line_index else None
        if key == "City":
...
# Ensure the "EMERSON_JOBS" folder exists relative to the script's location
folder_path = os.path.join(os.getcwd(), "EMERSON_JOBS")
if not os.path.exists(folder_path):
    os.makedirs(folder_path)
...
if not os.path.exists(csv_path):
    raise FileNotFoundError("The file 'ALL_JOBS.csv' was not found in the 'EMERSON_JOBS' folder.")
